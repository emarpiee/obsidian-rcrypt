...
const zh: TranslationSchema = {
	...
	defaultSaltDesc: '与密码一起使用的盐（对应于 rclone.conf 中的 password2）。留空将使用 rclone 内置的二进制默认盐。强烈建议设置自定义盐。',
	defaultSaltWarning: ' ⚠️ 使用默认盐在面对彩虹表攻击时较弱。留空代表 rclone 内置默认值（不是字符串 "rclone"）。',
...
const zhTW: TranslationSchema = {
	...
	defaultSaltName: 'Password or pass phrase (for salt)',
	defaultSaltDesc: '與密碼一起使用的鹽（對應於 rclone.conf 中的 password2）。留空將使用 rclone 內建的二進位預設鹽。強烈建議設定自訂鹽。',
	defaultSaltWarning: ' ⚠️ 使用預設鹽在面對彩虹表攻擊時較弱。留空代表 rclone 內建預設值（不是字串 "rclone"）。',
...
const es: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ Usar la sal predeterminada es menos seguro. Dejar en blanco equivale al valor binario predeterminado de rclone (no la cadena "rclone").',
...
const fr: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ L’utilisation du sel par défaut est moins sécurisée. Laisser vide correspond à la valeur binaire par défaut de rclone (pas la chaîne "rclone").',
...
const de: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ Die Verwendung des Standard-Salts ist schwächer. Leer lassen entspricht dem binären Standardwert von rclone (nicht der Zeichenkette "rclone").',
...
const ja: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ デフォルトのソルトの使用は安全性が低くなります。空白は rclone のバイナリデフォルト値に相当します（文字列 "rclone" ではありません）。',
...
const ko: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ 기본 솔트 사용은 보안에 취약합니다. 비워두면 rclone의 이진 기본값에 해당합니다 (문자열 "rclone"이 아님).',
...
const ru: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ Использование соли по умолчанию менее безопасно. Пустое поле соответствует бинарному значению rclone по умолчанию (не строке "rclone").',
...
const ar: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ استخدام الملح الافتراضي أضعف ضد هجمات جدول التشفير. تركه فارغًا يعادل القيمة الثنائية الافتراضية لـ rclone (وليس النص "rclone").',
...
const he: TranslationSchema = {
	...
	defaultSaltWarning: ' ⚠️ שימוש ב-Salt ברירת מחדל חלש יותר. שדה ריק שווה ערך לברירת המחדל הבינארית של rclone (לא למחרוזת "rclone").',
